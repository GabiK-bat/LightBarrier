...
dat$Site=gsub("Eldena_L8","Eldena_L16k",dat$Site)
dat$Site=gsub("Peenemuende","Peenemünde",dat$Site)
...
reg_pass=
  ...
  annotate("text", x =9, y=24, label=c(expression(paste("R"^"2","= 0.79, p = 0.005"))))+
  annotate("text", x =9, y=21, label=c(expression(paste("Est = 0.83"))))+
  geom_text_repel(aes(label=Site))
#0.83 missed pass per 100 pass ("chunk")
#estimate total missed passes: chunk*0.83*(total passes/population size estimate)
reg_pass

ggsave(paste0("./accuracy/output/regression_missedpass_new.png"),
       width =12, height =12, dpi = 800, units = "cm", device='png')
```

```{r jackknife_approach}
#leave out one point and re-do regression
dat
for (i in 1:nrow(dat)){
  Site=dat$Site[i]
  dat_temp=dat[-i,]
  mod=summary(lm(missed_pass~0+ratio, data=dat_temp))
  print(paste("Excluded site:", Site))
  r=round(mod$r.squared,2)
  print(paste("Adjusted R-squared:", r))
  p=round(mod$coefficients[4],3)
  print(paste("P-value:", p))
  est=round(mod$coefficients[1],2)
...
ggsave(paste0("./accuracy/output/regression_SD_new.png"),
       width =12, height =12, dpi = 800, units = "cm", device='png')
```
...
for (i in 1:nrow(dat)){
  Site=dat$Site[i]
  dat_temp=dat[-i,]
  mod=summary(lm(I(sd_error - intercept) ~ 0 + ratio, data=dat_temp))
  print(paste("Excluded site:", Site))
  r=round(mod$r.squared,2)
  print(paste("Adjusted R-squared:", r))
  p=round(mod$coefficients[4],3)
